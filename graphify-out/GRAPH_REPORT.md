# Graph Report - Elyra-main  (2026-07-02)

## Corpus Check
- 153 files · ~130,996 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1075 nodes · 2057 edges · 97 communities (81 shown, 16 thin omitted)
- Extraction: 60% EXTRACTED · 40% INFERRED · 0% AMBIGUOUS · INFERRED: 817 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c7a5abe8`
...
- `AIClient` --calls--> `ai_client()`  [INFERRED]
  codebase/app/backend/core/ai_client.py → codebase/app/backend/tests/ai/test_embedding.py
- `AIClient` --uses--> `Tests for the moderation AI service (respx-mocked).`  [INFERRED]
  codebase/app/backend/core/ai_client.py → codebase/app/backend/tests/ai/test_moderation.py
- `AIClient` --uses--> `Tests for the embedding AI service (respx-mocked).`  [INFERRED]
  codebase/app/backend/core/ai_client.py → codebase/app/backend/tests/ai/test_embedding.py
...
Cohesion: 0.05
Nodes (85): AIClient, Extract and validate JWT from the socket.io auth dict.      auth dict comes from, Base, Base, TimestampMixin, DeclarativeBase, UserEmbedding, EventPublisher (+77 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (33): AuditLog, change_password(), forgot_password(), login(), logout(), oauth_login(), refresh(), register() (+25 more)
...
Cohesion: 0.05
Nodes (30): get_db(), FakeProfileDetector, health_detailed(), liveness(), Always 200 - proves the process is alive., Checks all three databases., readiness(), configure_logging() (+22 more)

### Community 4 - "Community 4"
Cohesion: 0.07
Nodes (17): _cached_encode(), EmbeddingModel, decrypt_field(), encrypt_field(), get_encryption_key(), Resolve the AES-256 key from settings.      Accepts the key as:       * 64-char, Encrypt with AES-256-GCM, prefixing the 12-byte random nonce.      Layout: ``non, Security: verify private profile fields are stored encrypted at rest. (+9 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (17): BaseHTTPMiddleware, Elyra FastAPI application entrypoint., discover(), like_user(), pass_user(), unmatch(), _compute_match_score(), _haversine_km() (+9 more)
...
Cohesion: 0.22
Nodes (26): AnonymousMessageFilter, Handles anonymous identity mapping for chat threads.     Each user gets a stable, ChatThread, ChatService, MongoDB CRUD operations for chat messages., Exception, socket.io event handlers. All handlers are registered on the `sio` instance from, Called via asyncio.create_task — never blocks message delivery. (+18 more)

### Community 8 - "Community 8"
Cohesion: 0.09
Nodes (24): WebSocket JWT auth helper. Used exclusively by the socket.io connect event., Extract and validate JWT from the socket.io auth dict.      auth dict comes from, verify_socket_token(), handle_get_online_status(), Input: { "user_ids": [str] }   (max 50 user_ids per call), connect(), disconnect(), get_redis() (+16 more)
...
Cohesion: 0.1
Nodes (16): admin_headers(), auth_headers(), client(), httpx_response(), moderator_headers(), _register_and_get_headers(), second_user_headers(), _get_client() (+8 more)
...
Cohesion: 0.14
Nodes (13): block_user(), check_in(), create_report(), create_safe_session(), end_session(), get_active_session(), list_reports(), list_sessions() (+5 more)
...
Cohesion: 0.17
Nodes (15): Add user_id to read_by array for all specified message_ids.         Use $addToSe, get_redis(), handle_join_thread(), handle_leave_thread(), handle_mark_read(), handle_send_message(), handle_typing_start(), handle_typing_stop() (+7 more)
...
Cohesion: 0.21
Nodes (14): ChangePasswordRequest, ForgotPasswordRequest, LoginRequest, OAuthLoginRequest, _password_strength(), ResetPasswordRequest, SendOTPRequest, TokenRefreshRequest (+6 more)
...
Cohesion: 0.15
Nodes (6): Set is_moderated=True and moderation_result on the message.         Query by _id, Soft delete: set is_deleted=True.         Only allowed if sender_id matches user, Called once at app startup.         Create the following indexes:           1. C, Insert message document into MongoDB.         Returns the inserted _id as string, Idempotency check.         Query: { thread_id, sender_id, client_message_id }, Fetch paginated messages for a thread.         Filter: { thread_id, is_deleted:
...
- **Why does `TrustSafetyService` connect `Community 0` to `Community 1`, `Community 36`, `Community 7`, `Community 12`, `Community 14`, `Community 25`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `EventSubscriber` connect `Community 0` to `Community 3`, `Community 5`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `AIClient` connect `Community 0` to `Community 18`, `Community 3`, `Community 12`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
